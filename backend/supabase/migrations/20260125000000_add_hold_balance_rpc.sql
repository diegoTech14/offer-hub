...
-- Description: Atomic function to move funds from available to held balance
...
BEGIN
    ...
    IF v_available_before IS NULL THEN
        RAISE EXCEPTION 'Insufficient available balance: user has no balance record for currency %', p_currency;
    END IF;

    -- 3. Validate sufficient available funds
    IF v_available_before < p_amount THEN
        RAISE EXCEPTION 'Insufficient available balance: required %, but only % available', p_amount, v_available_before;
    END IF;

    -- 4. Update Balance Record (decrement available, increment held)
    UPDATE balances
    SET
    ...
    VALUES
    (p_user_id, p_amount, p_currency, 'hold', p_ref_id, p_ref_type, v_available_before, v_available_before - p_amount, p_description);
